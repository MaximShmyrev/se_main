	...
	*/
	...
	SceneBuilder.prototype._addGeometryToNode = function(node, geometry, submeshInfo) {
		...
		var path = new Path({
			...
			fillStyle: geometry.isPolyline ? null : { color: material.color },
			subelement: true
		});
		path.uid += "-g";
		node.add(path);
	 ...
	 * @param {any} sceneId The scene identifier
	 * @param {any} lineStyles Line styles
	 * @param {any} fillStyles Fill styles
	 * @public
	 */
	SceneBuilder.prototype.setParametricContent = function(nodeId, parametricContent, sceneId, lineStyles, fillStyles) {
		...
		if (shapes) {
			...
			for (var i = 0; i < shapes.length; i++) {
				var shape = this._createObject(shapes[ i ], lineStyles, fillStyles);
				if (shape) {
					node.add(shape);
				}
			}
		} else {
			var newNode = this._createObject(parametricContent, lineStyles, fillStyles);
			if (newNode) {
				node.add(newNode);
			}
		}
	...
	SceneBuilder.prototype._createObject = function(parametricContent, lineStyles, fillStyles) {
		...
		if (lineStyles && parametricContent.stroke_id !== undefined) {
			parametricContent.lineStyle = lineStyles.get(parametricContent.stroke_id);
		}

		if (fillStyles && parametricContent.fill_id !== undefined) {
			parametricContent.fillStyle = fillStyles.get(parametricContent.fill_id);
		}
	...
	/**
	 * Retrieve map of fill styles.
	 * @public
	 * @returns {Map} Map with fill styles, key is fill style veid
	*/
	SceneBuilder.prototype.getFillStyles = function() {
		return this._fillStyles;
	};

	/**
	 * Retrieve map of line styles.
	 * @public
	 * @returns {Map} Map with line styles, key is line style veid
	*/
	SceneBuilder.prototype.getLineStyles = function() {
		return this._lineStyles;
	};

	SceneBuilder.prototype.createImage = function(imageInfo) {
		Log.warning("SceneBuilder.createImage() is not implemented", imageInfo);
	};

	SceneBuilder.prototype.setViewThumbnail = function(imageId, viewId, sceneId, tileWidth) {
		Log.warning("SceneBuilder.setViewThumbnail() is not implemented");
	};
