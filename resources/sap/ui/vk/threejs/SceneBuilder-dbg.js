	...
	SceneBuilder.prototype.insertHighlightStyle = function(info) {
		...
		if (info.duration > 0) {
			if (info.colours && info.colours.length === 1 && (!info.opacities || info.opacities.length === 1)) {
				// When a hightlight has a duration but only a single color and single / no opacity,
				// assume color hightlight flashing, so insert a second color with same RGB components
				...
				// When a hightlight has a duration but only a single opacity and no color,
