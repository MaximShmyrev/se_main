...
sap.ui.define([
	...
	Annotation.prototype._setExpandText = function(textValue) {
		...
		for (var i = 0; i < length; i++) {
			...
			if (textValue[i] !== ">") {
				var delay = (this.getAnimationDelay() + 0.5 + "s");
				var animationName = this.getAnimate() ? "annotationExpandTextSpan" : "annotationStatic";
				newText += "<span style='animation: " + animationName + " " + ((0.1 / length) * (i + 1)) + "s linear " + delay + " ;'>";
				newText += textValue[i];
				newText += "</span>";
			}
		}
