...
sap.ui.define([
	...
	var SmartTable = VBox.extend("sap.ui.comp.smarttable.SmartTable", /** @lends sap.ui.comp.smarttable.SmartTable.prototype */ {
		metadata: {
			...
			properties: {
				...
				/**
				 * When set to true, this enables heuristic column width calculation based on metadata information.
				 *
				 * <b>Note:</b> Enabling this property enforces the following changes
				 * <ul>
				 *	<li>{@link sap.ui.comp.smarttable.SmartTable#getDemandPopin demandPopin} property of the <code>SmartTable</code> is set to <code>true</code></li>
				 * 	<li>{@link sap.m.Table#getFixedLayout fixedLayout} property of the <code>sap.m.Table</code> is set to <code>Strict</code></li>
				 * 	<li>{@link sap.m.Table#getContextualWidth contextualWidth} property of the <code>sap.m.Table</code> is set to <code>auto</code></li>
				 * </ul>
				 * Those properties should not be managed by the application.
				 *
				 * <b>Note:</b> This is not a dynamic property and cannot be changed once the control has been initialized.
				 *
				 * @experimental since 1.86. This API is experimental and subject to change.
				 * @since 1.86
				 */
	...
	SmartTable.prototype.getEnableAutoColumnWidth = function() {
		if (document.location.search.includes("sap-ui-xx-enableAutoColumnWidth")) {
			return true;
		}

		return this.getProperty("enableAutoColumnWidth");
	};
	 ...
	 * @param {integer} [iMin=0] The minimum text length value to measure the given text
	 * @returns {float} The text width
	 * @private
	 */
	function measureText(sText, iMin) {
		iMin = iMin || 0;
		sText = sText || "";

		if (sText.length <= iMin) {
			return iMin;
		}

		if (!SmartTable.oCanvasContext) {
			var fnUpdateFont = function() {
				SmartTable.oCanvasContext.font = ThemeParameters.get("sapMFontMediumSize") + " " + ThemeParameters.get("sapUiFontFamily");
				delete SmartTable.fBooleanWidth;
			};

			SmartTable.oCanvasContext = document.createElement("canvas").getContext("2d");
			Core.attachThemeChanged(fnUpdateFont);
			fnUpdateFont();
		}

		return SmartTable.oCanvasContext.measureText(sText).width / parseFloat(MLibrary.BaseFontSize);
	}
	 ...
	 * @param {integer} [mConfig.max=20] The maximum width of the column
	 * @param {integer} [mConfig.min=3] The minimum width of the column
	 * @param {boolean} [mConfig.label=true] Whether the column label should be taken into account as minimum column width or not
	 * @param {boolean} [mConfig.labelSplit=false] Whether the longest word in column label should be taken into account as minimum column width or not
	 * @param {integer} [mConfig.defaultWidth=8] The default column width when type check fails or field metadata info not found
	 * @param {integer} [mConfig.padding=1] The additional width added to the content calculation of the column
	...
	SmartTable.prototype._calcColumnWidth = function(oField, bAdditionalProperty, mConfig) {
		mConfig = Object.assign({
			max: 20,
			min: bAdditionalProperty ? 2 : 3,
			label: bAdditionalProperty ? false : true,
			labelSplit: this._isMobileTable,
			defaultWidth: bAdditionalProperty ? 4 : 8,
			epsilon: bAdditionalProperty ? 0 : 0.05,
			padding: this._isMobileTable ? 1 : 1.25
		}, mConfig);
		...
		var vSample,
			...
			iMin = mConfig.min,
			iMax = mConfig.max,
			fPadding = mConfig.padding,
			sDisplayBehaviour = oField.displayBehaviour,
			sType = oField.type || "",
			mNumericLimits = {
				"Edm.Int16" : 6,    // longest possible value -32768
				"Edm.Int32" : 11,	// longest possible value -2147483648
				"Edm.Int64" : 13,	// we do not expect 64-bit integer to be used therefore limited to 9,999,999,999,999
				"Edm.Float" : 7,	// represents a floating point number with 7 digits precision
				"Edm.Single" : 7,	// represents a floating point number with 7 digits precision
				"Edm.Double" : 14,	// represents a floating point number with 14 digits precision
				"Edm.Decimal" : 17  // we do not expect 2^255 to be used therefore limited to 17 digits precision
			};

		if (bAdditionalProperty || sDisplayBehaviour != "descriptionOnly") {
			if (oField.isImageURL || sType == "Edm.Byte" || sType == "Edm.SByte") {
				fWidth = iMin;
			} else if (sType.startsWith("Edm.Date") || sType.startsWith("Edm.Time")) {
				vSample = new Date(2023, 9, 26, 22, 47, 58); // for the possible logest(formatted) date we should avoid using "1" as much as possible
				fPadding += 0.5; // in case of day or month names are used as a formatting option we just add a few more pixels not to truncate dates
				if (sType == "Edm.Time") {
					var iTimezoneOffset = vSample.getTimezoneOffset() * 60 * 1000; // get the timezone offset in miliseconds
					vSample = DateTimeUtil.dateToEdmTime(vSample); // convert Date object to Edm.Time object as it is used in the ODataModel
					vSample.ms -= iTimezoneOffset; // time formatting uses always UTC but we need the local time format to ensure the longest possible time
				}
			} else if (sType in mNumericLimits) {
				var iPrecision = Math.min(oField.precision || 20, mNumericLimits[sType]);
				vSample = Math.pow(10, iPrecision - (oField.scale || 0) - 1); // create a numeric sample from the precision and scale
			}  else if (sType == "Edm.Boolean") {
				// the text of boolean values are static therefore it is enough calculate the longest one only once
				if (!SmartTable.hasOwnProperty("fBooleanWidth")) {
					var fTrueWidth = measureText(oField.modelType.formatValue(true, "string"), iMin);
					var fFalseWidth = measureText(oField.modelType.formatValue(false, "string"), iMin);
					SmartTable.fBooleanWidth = Math.max(fTrueWidth, fFalseWidth);
				}
				fWidth = SmartTable.fBooleanWidth;
			} else if (oField.type == "Edm.String") {
				var iMaxLength = +oField.maxLength || 0; // calculate the size with taking the maxLength into account
				if (iMaxLength <= iMin) {
					fWidth = iMin; // assumption: 1 character < 1rem
				} else if (iMaxLength < 15) {
					vSample = sChar.repeat(iMaxLength); // up to 15 characters we take the longest possible word as a reference
				} else {
					var fSampleWidth = measureText(sChar.repeat(iMaxLength), iMin);
					fWidth = Math.log10(fSampleWidth) * (iMax / 2); // for larger maxLength we use logarithma to chop the long texts
				}
			} else {
			...
			if (vSample) {
				// let the model type format the possible value with taking constraints and formatting options into account
				// 1000000 vs 1.000.000   or   2023.10.26 vs Oct 26, 2023   or   22:47:58 vs 10:47 PM
				var sSample = oField.modelType.formatValue(vSample, "string");
				fWidth = measureText(sSample, iMin);
			}
		}
		...
		if (!bAdditionalProperty && sDisplayBehaviour != "idOnly" && oField.additionalProperty && this._oTableProvider) {
			oField.additionalProperty.split(",").forEach(function(sAdditionalProperty) {
				var mAdditionalField = this._oTableProvider.getFieldMetadata(sAdditionalProperty);

				// for horizontal templates the additional value is shown within parenthesis and there is a space between, therefore we spare 1rem
		...
		if (mConfig.label) {
			var sLabel = (oField.label || oField.fieldLabel || "").trim();
			var iLabelLength = sLabel.length;

			// The label size calculation is not necessary if;
			//  - the calculated field width already exceeds the maximum column width
			//  - the calculated filed width is too large and the label length is too small (assumption: 1 character < 1rem)
			//  - label length is smaller than the minimum column width (assumption: 1 character < 1rem)
			if (fWidth < iMax && fWidth < iLabelLength && iLabelLength > iMin) {
				if (mConfig.labelSplit && iLabelLength > iMax) {
					sLabel = sLabel.split(" ").reduce(function (sLongestWord, sWord) {
					    return (sLongestWord.length > sWord.length) ? sLongestWord : sWord;
					}, "");

					// longest word with narrow characters can be shorter than other words with wide characters
					if (sLabel.length < iLabelLength / 2) {
						sLabel += sChar;
					}
				}

				fLabelWidth = measureText(sLabel, iMin);
			}
		}

		fWidth = Math.max(iMin, fWidth, fLabelWidth);
		fWidth = Math.min(fWidth + fPadding + mConfig.epsilon, iMax);
	...
	SmartTable.prototype._createColumn = function(oField, sId) {
		var oColumn = new Column(sId, {
			autoResizable: true,
			hAlign: oField.align,
			width: oField.width || this._calcColumnWidth(oField) + "rem",
	...
	SmartTable.prototype._createMobileColumn = function(oField, sId) {
		...
		var iMax = 20, fWidth = this._calcColumnWidth(oField);
		if (fWidth == iMax) {
			oColumn.setWidth("auto");
			oColumn.setAutoPopinWidth(iMax);
		} else {
			oColumn.setWidth(fWidth + "rem");
		}
