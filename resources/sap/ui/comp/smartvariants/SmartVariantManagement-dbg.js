...
//				} else
				...
				this._oPersistencyPromise.then(function() {
					...
					if (this._oControlPromise && this._oPersoControl && oCurrentControlWrapper) {
						Promise.all([
							...
							var bIsVariantCreationEnabled = true;
							this._dataReceived(aVariants[1], aVariants[2], bIsVariantCreationEnabled, oCurrentControlWrapper);
	...
	SmartVariantManagement.prototype._dataReceived = function(mVariants, bIsVariantSharingEnabled, bIsVariantCreationEnabled, oCurrentControlWrapper) {
	...
	SmartVariantManagement.prototype._initialize = function(parameter, oCurrentControlWrapper) {
		...
		SyncPromise.resolve(oResult).then(function(oResult) {
			...
			if ((this._getCurrentVariantId() === this.getStandardVariantKey()) && this.getExecuteOnSelectForStandardVariant()) {
				if (oCurrentControlWrapper.control.search /* && !this.getExecuteOnSelectionForStandard() */) {
...
//			if (this.getExecuteOnSelectionForStandard() && (this.getStandardVariantKey() === sVariantKey)) {
//				oContent.executeOnSelection = undefined;
//			}
