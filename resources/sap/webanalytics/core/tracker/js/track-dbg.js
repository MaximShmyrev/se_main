...
(function() {
  ...
  swa.variableInit = function() {
    ...
    // If provided, validate the URL
      ...
      Console.error(
        "URL Validation failed for privacy link."
      );
      swa.privacyLink = null;
    }
  ...
  //Indicating that the preferences API returned 200 response
    ...
    if (swa.cookieTimeoutForVisitor === -1 && swa.sessionCookieTimeout > 1800) {
      swa.sessionCookieTimeout = 1800;
      swa.cookieTimeoutForVisitor =
        (new Date("2099-12-31 23:59:59") - new Date()) / 1000; //Set cookie timeout as some future value as it is not relevant and visitor is reset for every new session,here, 31st Dec 2099
      swa.setAsSession = 1;
      Console.warn(
        "Resetting session timeout to 30 minutes. Session timeout cannot be more than 30 minutes if Set as Session in Site Management is turned on."
      );
    } else if (
    ...
    if (
      (siteProperties.internalApplicationUser &&
        !siteProperties.bannerEnabled &&
        siteProperties.globalBannerEnabled) ||
      (!siteProperties.internalApplicationUser && !siteProperties.bannerEnabled)
    ) {
    ...
    if (
      ...
      if (
        ...
        if (swa.setAsSession) {
          ...
          if (
            siteProperties.bannerEnabled &&
            swa.getCookieValue("_swa_tracking_consent." + swa.pubToken) !=
              "1" &&
            swa.consentOnLoad &&
            !swa.isConsentGiven
          ) {
            ...
            var createdTime = swa
              .getCookieValue("_swa_tracking_consent." + swa.pubToken)
              .slice(2);
    ...
    if (swa._isUI5() && parseFloat(sap.ui.version) >=1.86 && !swa.getCookieValue("_swa_pref." + swa.pubToken)) {
      ...
      if(pref.cri && pref.cri!=0){
        ...
        swa.pubToken +
        "=" +
        encodeURIComponent(JSON.stringify(pref)) +
        "; expires=" +
        expDate.toUTCString();
        ...
        if (data) {
         ...
         if(loggingEnabled == "A"){
          ...
          for (var i = 0; i < reportedParamPairs.length; i++) {
            ...
            reportedParams[
              decodeURIComponent(reportedParamPair[0].replace(/\+/g, "%20"))
            ] = decodeURIComponent(reportedParamPair[1].replace(/\+/g, "%20"));
          }
          ...
          if((!siteProperties.visitorUUIDAnonymity &&
            swa.setAsSession &&
            reportedParams["_idvc"] > "1") || (loggingEnabled == 'A' && reportedParams["_idvc"] > "1") ){
          ...
          if (rebuildData === true) {
            ...
            for (var paramName in reportedParams) {
              if (
                Object.prototype.hasOwnProperty.call(reportedParams, paramName)
              ) {
    ...
    if (
      ...
      if (
        ...
        if (
          !isInvalidFormat &&
          !(
            stringCustomEventvalues.length < 1 ||
            stringCustomEventvalues[0] == null ||
            stringCustomEventvalues[0] == undefined
          )
        )
          Piwik.getAsyncTracker().trackEvent(
            eventType,
            stringCustomEventvalues[0]
          );
        else
          Console.log(
            "Bad usage of swa.trackCustomEvent() api.  Invalid parameters used in the API"
          );
      }
    } else {
      if (
...
banner_save(event);
 return false;
  ...
  function localizeText() {
    ...
    var i18nurl = "";
    if (
      (typeof swa.consentOptInText !== undefined ||
        typeof swa.consentOptOutText !== undefined || typeof swa.anonymousConsentText!== undefined ||typeof swa.anonymousConsentOptOutText !== undefined) &&
      count < 1
    ) {
  ...
  function getTranslatedText(oBundle, isUI5) {
   ...
   : decodeURIComponent(JSON.parse('"' +  oBundle.TRACKING_ANON_STATUS.replace(/\"/g, '\\"') + '"'));
    ...
    if (typeof swa.anonymousConsentText === undefined)
      swa.anonymousConsentText = isUI5
      ? oBundle.getText('ANONYMOUS_CONSENT_OPT_IN_TEXT')
      : decodeURIComponent(JSON.parse('"' +  oBundle.ANONYMOUS_CONSENT_OPT_IN_TEXT.replace(/\"/g, '\\"') + '"'));

    if (typeof swa.anonymousConsentOptOutText === undefined)
  ...
  function getDefaultTextForConsent() {
    swa.consentOptInText = (swa.consentOptInText === undefined)?(i18nValues[0] == undefined ?defaultConsentValues[0]:i18nValues[0]):swa.consentOptInText;
      ...
      swa.consentAllowButton =(swa.consentAllowButton === undefined)? (i18nValues[2] == undefined ?defaultConsentValues[2]:i18nValues[2]):i18nValues[2];
      swa.consentDenyButton = (swa.consentDenyButton === undefined)? (i18nValues[3] == undefined ?defaultConsentValues[3]:i18nValues[3]):i18nValues[3];
      swa.consentStopButton = (swa.consentStopButton === undefined)? (i18nValues[4] == undefined ?defaultConsentValues[4]:i18nValues[4]):i18nValues[4];
      swa.consentCloseButton = (swa.consentCloseButton === undefined)? (i18nValues[5] == undefined ?defaultConsentValues[5]:i18nValues[5]):i18nValues[5];
      swa.privacyLinkText = (swa.privacyLinkText === undefined)? (i18nValues[6] == undefined ?defaultConsentValues[6]:i18nValues[6]):i18nValues[6];
      swa.anonymizeButtonText = (swa.anonymizeButtonText === undefined)? (i18nValues[9] == undefined ?defaultConsentValues[9]:i18nValues[9]):i18nValues[9];
      swa.customConsentOptionText = (swa.customConsentOptionText === undefined)? (i18nValues[10] == undefined ?defaultConsentValues[10]:i18nValues[10]):i18nValues[10];
      swa.trackOnStatus = (swa.trackOnStatus === undefined)? (i18nValues[11] == undefined ?defaultConsentValues[11]:i18nValues[11]):i18nValues[11];
      swa.trackOffStatus = (swa.trackOffStatus === undefined)? (i18nValues[12] == undefined ?defaultConsentValues[12]:i18nValues[12]):i18nValues[12];
      swa.trackAnonymousStatus = (swa.trackAnonymousStatus === undefined)? (i18nValues[13] == undefined ?defaultConsentValues[13]:i18nValues[13]):i18nValues[13];
      swa.trackingStatusText = (swa.trackingStatusText === undefined)? (i18nValues[14] == undefined ?defaultConsentValues[14]:i18nValues[14]):i18nValues[14];
