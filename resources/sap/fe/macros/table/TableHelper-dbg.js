		...
		var TableHelper = {
			...
			_isStaticAction: function(oActionContext, sActionName) {
				...
				if (sActionName && sActionName.indexOf("(") > -1) {
					...
					var sEntityType = aParts[1].slice(0, -1);
					...
					if (
						...
						oAction
							.getModel()
							.getMetaModel()
							.getMetaContext(oAction.getPath())
				...
				if (oAction && oAction.$IsBound && oAction.$Parameter[0].$isCollection) {
					return true;
				}
			},
			...
			isDataFieldForActionEnabled: function(oThis, oDataField, oRequiresContext, bIsDataFieldForIBN, oActionContext, sActionEnabled) {
				...
				if (bIsDataFieldForIBN) {
					...
					var sAction = "${internal>dynamicActions/" + oDataField.Action + "/bEnabled" + "}";
