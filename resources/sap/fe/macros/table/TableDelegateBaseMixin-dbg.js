...
sap.ui.define(
	...
	function(
		...
		var TableDelegateBaseMixin = {
			_fetchPropertyInfo: function(oBindingContext, oColumnInfo) {
				var sAbsoluteNavigationPath = oColumnInfo.annotationPath,
					...
					isComplexProperty = oColumnInfo.propertyInfos !== undefined,
					...
					oExportSettings = {
						label: (oColumnInfo.exportSettings && oColumnInfo.exportSettings.label) || oColumnInfo.label,
						type: sExportType,
				...
				var oPropertyInfo = {
					...
					label: oColumnInfo.label,
					description: sDescription || oColumnInfo.label,
				...
				if (oColumnInfo.propertyInfos && oColumnInfo.propertyInfos.length) {
					...
					oPropertyInfo.sortable = oColumnInfo.sortable && !oColumnInfo.relativePath.includes("/");
					oPropertyInfo.filterable = bFilterable;
			...
			_fetchCustomPropertyInfo: function(oColumnInfo, oTable) {
				var sLabel = DelegateUtil.getLocalizedText(oColumnInfo.header, oTable); // Todo: To be removed once MDC provides translation support
				var oPropertyInfo = {
					name: oColumnInfo.name,
					groupLabel: null,
					group: null,
			...
			_fetchPropertiesForEntity: function(oTable, sBindingPath, oMetaModel) {
				...
				return Promise.resolve(_getColumnsFor(oTable)).then(function(aColumns) {
					...
					aColumns.forEach(function(oColumnInfo) {
						switch (oColumnInfo.type) {
							case "Annotation":
								oPropertyInfo = that._fetchPropertyInfo(oBindingContext, oColumnInfo);
								...
								break;
							case "Default":
								oPropertyInfo = that._fetchCustomPropertyInfo(oColumnInfo, oTable);
								break;
							default:
			...
			_getCachedOrFetchPropertiesForEntity: function(oTable, sEntitySetPath, oMetaModel) {
				var aFetchedProperties = _getCachedProperties(oTable);

				if (aFetchedProperties) {
					return Promise.resolve(aFetchedProperties);
				}
				return this._fetchPropertiesForEntity(oTable, sEntitySetPath, oMetaModel).then(function(aFetchedProperties) {
			...
			_getVHRelevantFields: function(oMetaModel, sMetadataPath, sBindingPath) {
				var aFields = [];
				switch (oMetaModel.getObject(sMetadataPath + "/$Type")) {
			...
			addItem: function(sPropertyInfoName, oTable, mPropertyBag) {
				...
				return this._getCachedOrFetchPropertiesForEntity(oTable, sPath, oMetaModel).then(function(aFetchedProperties) {
