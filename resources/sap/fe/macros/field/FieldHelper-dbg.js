...
sap.ui.define(
	...
	function(
		...
		var FieldHelper = {
			 ...
			 */
			getConditionsBinding: function(iContext, vProperty) {
				var oMetaModel = iContext.getModel(),
					sFullPropertyPath = iContext.getPath(),
					aPropertyPathParts = sFullPropertyPath.substring(1).split("/"),
					sEntitySet = aPropertyPathParts.shift(),
					sConditionPath = "",
					vProperty,
					i;

				for (i = 0; i < aPropertyPathParts.length; ++i) {
					vProperty = oMetaModel.getProperty("/" + sEntitySet + "/" + aPropertyPathParts.slice(0, i + 1).join("/"));
			...
			isDateTimeDataType: function(sDataFieldType) {
				if (sDataFieldType !== undefined) {
					var aDateTimeDataTypes = ["Edm.DateTimeOffset", "Edm.Date", "Edm.TimeOfDay"];
					return aDateTimeDataTypes.indexOf(sDataFieldType) > -1;
				} else {
					return false;
				}
			},
			isDateDataType: function(sDataFieldType) {
				if (sDataFieldType !== undefined) {
					var aDateDataTypes = ["Edm.DateTimeOffset", "Edm.Date"];
					return aDateDataTypes.indexOf(sDataFieldType) > -1;
				} else {
					return false;
				}
			},
			isTimeDataType: function(sDataFieldType) {
				return sDataFieldType === "Edm.TimeOfDay";
			},
			...
			getTypeAlignment: function(oContext, oDataField, oFormatOptions, sEntityPath, oComputedEditMode) {
				var oInterface = oContext.getInterface(0);
			...
			getMultipleLinesForPropertyField: function(oThis, oProperty) {
				...
				if (oThis.editMode.indexOf("{") > -1) {
					// If the editMode is computed (field control...) then we only care about the page editMode to see if the field is multiline or not
					return "{= ${ui>/editMode} === 'Display' ? true : " + oProperty["@com.sap.vocabularies.UI.v1.MultiLineText"] + "}";
				}
				return oProperty["@com.sap.vocabularies.UI.v1.MultiLineText"];
			},
				...
				var oAnnotation,
					...
					getValueListPropertyName = function(oValueList) {
						var oValueListParameter = oValueList.Parameters.find(function(oParameter) {
							return oParameter.LocalDataProperty && oParameter.LocalDataProperty.$PropertyPath === oProperty.$Name;
						});
			...
			getEmptyIndicatorTrigger: function(bActive, sBinding) {
