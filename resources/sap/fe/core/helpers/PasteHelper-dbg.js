	...
	var oPasteHelper = {
		_getColumnInfoForPaste: function(oTable) {
			...
			oTable.getColumns().forEach(function(oColumn) {
				var sDataProperty = oColumn.getDataProperty();
				if (!sDataProperty) {
					// Empty column --> ignore
					aColumnInfos.push({
						property: "unused",
					...
					var oProperty = oMetaContext.getProperty(sDataProperty),
						mFormatOptions = { parseKeepsEmptyString: true },
						oType = oProperty && oMetaModel.getUI5Type(sRowBindingPath + "/" + sDataProperty, mFormatOptions),
						bIgnore = !oProperty || oMetaContext.getProperty(sDataProperty + "@Org.OData.Core.V1.Computed");
					aColumnInfos.push({
					...
					var sAdditionalPropertyName =
						oMetaContext.getProperty(sDataProperty + "@Org.OData.Measures.V1.Unit/$Path") ||
						oMetaContext.getProperty(sDataProperty + "@Org.OData.Measures.V1.ISOCurrency/$Path");
