...
sap.ui.define(
	...
	function(
		...
		function callBoundAction(sActionName, contexts, oModel, mParameters) {
			...
			var oBoundAction = oMetaModel.createBindingContext(sActionPath + "/@$ui5.overload/0");
										...
										beforeOpen: function() {
											...
											var fnSetDefaultsAndOpenDialog = function(sBindingParameter) {
													...
													return new Promise(function(resolve, reject) {
														...
														if (vParamDefaultValue) {
															...
															if (oParameterModel && oParameterModel.oData[sParamName]) {
																resolve({ value: oParameterModel.oData[sParamName] });
															} else {
												...
												for (var i in mParameters.aActionParameters) {
													...
													aParmValues.push(prefillParameter(sParamName, vParameterDefaultValue));
												}

												Promise.all(aParmValues)
													.then(function(aValues) {
														for (var i = 0; i < aValues.length; i++) {
															if (aValues[i] && aValues[i].value) {
														...
														var bErrorFound = aValues.some(function(oValue) {
