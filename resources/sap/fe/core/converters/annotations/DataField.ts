 ...
 */
function _collectPropertiesFromAnnotations(
	property: PropertyAnnotationValue<Edm.PrimitiveType>,
	...
	if (property?.path) {
		const navigationPathPrefix = property.path.indexOf("/") > -1 ? property.path.substring(0, property.path.lastIndexOf("/") + 1) : "";

		// Check for Text annotation.
		const textAnnotation = property?.$target?.annotations?.Common?.Text;
		...
		if (textAnnotation?.path) {
			...
			switch (displayMode) {
				case "Value":
					valueIndex = _pushUnique(property.path, { value: property.$target });
				...
				case "Description":
					...
					_pushUnique(property.path, { value: property.$target });
			...
			const currencyOrUoMProperty = getAssociatedCurrencyProperty(property.$target) || getAssociatedUnitProperty(property.$target);
			const currencyOrUnitAnnotation =
				property?.$target?.annotations?.Measures?.ISOCurrency || property?.$target?.annotations?.Measures?.Unit;
