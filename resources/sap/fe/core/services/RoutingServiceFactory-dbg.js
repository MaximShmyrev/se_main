	...
	function(
		...
		var RoutingService = Service.extend("sap.fe.core.services.RoutingService", {
			...
			navigateToContext: function(oContext, mParameters, oViewData, oCurrentTargetInfo) {
				...
				if (mParameters.checkNoHashChange) {
					...
					if (sTargetPath === sCurrentHashNoAppState) {
						...
						this.eventProvider.fireEvent("routeMatched", mEventParameters);
						this.eventProvider.fireEvent("afterRouteMatched", mEventParameters);
			...
			_onRouteMatched: function(oEvent) {
				...
				EditState.cleanProcessedEditState(); // Reset UI state only when all bindings have been refreshed
			...
			initializeRouting: function() {
				...
				this.oRouter.attachRouteMatched(this._onRouteMatched.bind(this));
				...
				var bIsIappState =
					...
					oComponentData = that.oAppComponent.getComponentData(),
				...
				if (!bIsIappState && bHasStartUpParameters) {
					...
					if (oStartUpParameters.preferredMode && oStartUpParameters.preferredMode[0] === "create" && !sHash) {
						...
						initPromise = that._managePreferredModeCreateStartup();
					} else {
						initPromise = that._manageDeeplinkStartup(oStartUpParameters);
					}
				}
