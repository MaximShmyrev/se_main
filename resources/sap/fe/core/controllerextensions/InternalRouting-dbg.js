	...
	function(
		...
		return ControllerExtension.extend(
			"sap.fe.core.controllerextensions.InternalRouting",
			{
				...
				/**
				 * Sets the current view in fullscreen in a FCL layout.
				 *
				 * @param {object} oEvent event parameter
				 *
				 * @sap-restricted
				 */
				handleFCLFullScreen: function(oEvent) {
					...
					this.navigateToContext(oContext, { sLayout: sNextLayout });
				},

				/**
				 * Go back to a column view in a FCL layout.
				 *
				 * @param {object} oEvent event parameter
				 *
				 * @sap-restricted
				 */
				handleFCLExitFullScreen: function(oEvent) {
					var oContext = oEvent.getSource().getBindingContext(),
						sNextLayout = oEvent
							.getSource()
							.getModel("fclhelper")
				...
				/**
				 * Closes the column for the current view in a FCL layout.
				 *
				 * @param {object} oEvent event parameter
				 *
				 * @sap-restricted
				 */
				handleFCLClose: function(oEvent) {
					var oContext = oEvent.getSource().getBindingContext();
					this.navigateBackFromContext(oContext, { noPreservationCache: true });
				},
				...
				_onRouteMatched: function(oEvent) {
					...
					for (var sKey in mArguments) {
						var sValue =
							(mArguments[sKey] && typeof mArguments[sKey] === "string" && decodeURIComponent(mArguments[sKey])) ||
							mArguments[sKey];
					...
					onInit: function() {
						...
						this._oAppComponent
							...
							.then(function(oRoutingService) {
								...
								that._oRoutingService.attachRouteMatched(that._onRouteMatched.bind(that));
