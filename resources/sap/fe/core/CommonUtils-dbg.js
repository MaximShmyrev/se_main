...
sap.ui.define(
	[
		...
		"sap/fe/core/converters/controls/Common/Filter"
	],
	function(
		View,
		Component,
					...
					if (aSelectOptionsPropertyNames.indexOf(sParameterName) == -1) {
						if (sParameterName in oValidProperties) {
							var sParameter = oSelectionVariant.getParameter(sOrigParamName),
								oValue = FilterConverter.getTypeCompliantValue(sParameter, oValidProperties[sParameterName].$Type),
								oCondition;
				...
				if (sPropertyName in oValidProperties) {
					...
					if (CommonUtils.isPropertyFilterable(oMetaModel, "/" + sEntitySet, sPropertyName, true)) {
						...
						aConditions = aSelectOptions.reduce(function(aCumulativeConditions, oSelectOption) {
							var oValue1 = FilterConverter.getTypeCompliantValue(oSelectOption.Low, oValidProperties[sPropertyName].$Type),
								oValue2 = oSelectOption.High
									? FilterConverter.getTypeCompliantValue(oSelectOption.High, oValidProperties[sPropertyName].$Type)
									: undefined;
							if ((oValue1 !== undefined || oValue1 !== null) && oSelectOption.Option) {
								var oCondition = FilterConverter.createConditions(
