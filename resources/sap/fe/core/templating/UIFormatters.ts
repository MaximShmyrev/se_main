...
import { DataModelObjectPath, isPathUpdatable } from "sap/fe/core/templating/DataModelPathHelper";
...
type DisplayMode = "Value" | "Description" | "DescriptionValue" | "ValueDescription";
export const getDisplayMode = function(oPropertyPath: PropertyOrPath<Property>, oDataModelObjectPath: DataModelObjectPath): DisplayMode {
	...
	const oProperty = (isPathExpression(oPropertyPath) && oPropertyPath.$target) || (oPropertyPath as Property);
	const oEntitySet = oDataModelObjectPath.targetEntitySet;
	const oTextAnnotation = oProperty.annotations?.Common?.Text;
	const oTextArrangementAnnotation =
		(typeof oTextAnnotation !== "string" && oTextAnnotation?.annotations?.UI?.TextArrangement) ||
		oEntitySet?.entityType?.annotations?.UI?.TextArrangement;
