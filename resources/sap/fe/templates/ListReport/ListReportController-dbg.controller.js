...
sap.ui.define(
	...
	function(
		...
		return FEController.extend("sap.fe.templates.ListReport.ListReportController", {
			metadata: {
				methods: {
					getExtensionAPI: {
						"public": true,
			...
			onInit: function() {
				FEController.prototype.onInit.apply(this);
				var that = this;
			...
			/**
			 * Method to update Page local UI Model with Filter Bar not applicable fields (specific to Multi EntitySets scenario).
			 *
			...
			_updateMultiNotApplicableFields: function(oInternalModelContext, oFilterBar) {
				...
				aTables.forEach(function(oTable) {
					...
					if (!mTabs.ignoredFields[sTableEntitySet]) {
						mTabs.ignoredFields[sTableEntitySet] = FilterUtils.getNotApplicableFiltersForEntity(oFilterBar, sTableEntityPath);
					}
				});
				oInternalModelContext.setProperty("tabs", mTabs);
			},
