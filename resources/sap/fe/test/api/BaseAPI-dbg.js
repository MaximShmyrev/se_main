sap.ui.define(
	["sap/fe/test/Utils", "sap/ui/test/OpaBuilder", "sap/fe/test/builder/FEBuilder", "sap/ui/mdc/p13n/panels/BasePanel"],
	function(Utils, OpaBuilder, FEBuilder, MdcP13nBasePanel) {
		"use strict";
		...
		BaseApi.prototype.createFieldMatcher = function(vFieldIdentifier, sDataFieldType) {
			...
			if (!Utils.isOfType(vFieldIdentifier, String)) {
				if (typeof vFieldIdentifier.dataField === "string") {
					...
					throw new Error(
						"not supported property parameter for creating a control id for a field: " + vFieldIdentifier.dataField
					);
				}
			} else {
		...
		BaseApi.prototype.createFormElementMatcher = function(vFormElementIdentifier, sDataFieldType) {
			...
			if (!Utils.isOfType(vFormElementIdentifier, String)) {
				if (vFormElementIdentifier.dataField && typeof vFormElementIdentifier.dataField === "string") {
					var sId = "FormElement::" + sDataFieldType + "::" + vFormElementIdentifier.dataField;
					vMatcher = FEBuilder.Matchers.id(new RegExp(Utils.formatMessage("{0}$", sId)));
				} else {
