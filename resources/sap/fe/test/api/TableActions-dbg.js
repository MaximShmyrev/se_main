	...
	function(TableAPI, Utils, OpaBuilder, FEBuilder, Interactable, VMBuilder, SortOrder, Core, FilterFieldBuilder, APIHelper) {
		...
		Actions.prototype.iSelectAllRows = function() {
			...
			return this.prepareResult(
				oTableBuilder
					.doSelectAll()
					.description(Utils.formatMessage("Selecting all rows on table '{0}'", this.getIdentifier()))
					.execute()
			);
		};
		...
		/**
		 * Scroll up/down to the first/last row of the table.
		 *
		 * @param {object} [sDirection] a string indicating the direction of the scroll (either "up" or "down" (if empty the default is "down"))
		 * @returns {object} an object extending a jQuery promise.
		 *
		 * @public
		 * @sap-restricted
		...
		/**
		 * Execute the table fullscreen action.
		 *
		 * @returns {object} an object extending a jQuery promise.
		 *
		 * @public
