sap.ui.define(
	...
	function(FormAPI, Utils, OpaBuilder, FEBuilder, MacroFieldBuilder) {
		...
		function _getBuilderForFormElement(vOpaInstance, vLinkIdentifier, vAction) {
			return MacroFieldBuilder.create(vOpaInstance) // identifying the FormElement
		...
		/**
		 * Click a checkBox assigned to a form. The checkBox is identified by id.
		 *
		 * @param {object|string} vcheckBoxIdentifier The checkBox to be clicked. If passed as an object use the following pattern:
		 * <code><pre>
		 * 	{
		 * 		<[fieldGroup]>: <name of the field-group>
		 * 		<dataField>: <name of field related to the checkBox>
		 *  }
		 * </pre></code>
		 * If passed as string, the Id will be checked.
		 *
		 * @returns {object} an object extending a jQuery promise.
		 *
		 * @private
		 * @experimental
		 */
		FormActions.prototype.iClickCheckBox = function(vcheckBoxIdentifier) {
			var oFormBuilder = this.getBuilder();

			function getMatcher(oContext, vcheckBoxLinkIdentifier) {
				if (!Utils.isOfType(vcheckBoxLinkIdentifier, String)) {
					return vcheckBoxLinkIdentifier.fieldGroup
						? _getBuilderForFieldGroup(oContext, vcheckBoxLinkIdentifier).doOnChildren(
								_getBuilderForFormElement(oContext, vcheckBoxLinkIdentifier, OpaBuilder.Actions.press())
						  )
						: _getBuilderForFormElement(oContext, vcheckBoxLinkIdentifier, OpaBuilder.Actions.press());
				} else {
					return OpaBuilder.create(oContext)
						.hasType("sap.m.CheckBox")
						.hasId(vcheckBoxIdentifier)
						.doPress();
				}
			}
			...
			return this.prepareResult(
				oFormBuilder // on the SubForm
					.doOnChildren(getMatcher(this, vcheckBoxIdentifier))
					.description(Utils.formatMessage("Pressing checkBox '{0}'", vcheckBoxIdentifier))
					.execute()
			);
		};
