sap.ui.define(
	...
	function(
		...
		return function(oPageDefinition) {
					...
					ViewId,
					{
						actions: {
							...
							iOpenVHDialog: function(sFieldName) {
								return OpaBuilder.create(this)
									.hasId(FilterBarId + "::FilterField::" + sFieldName.replace(/\//g, "::") + "-inner-vhi")
									.doPress()
							...
							iConfirmVHDSelection: function(sFieldName, bSuppressApplyFiltersToList) {
								var oResult = OpaBuilder.create(this)
									.hasId(FilterBarVHDId + sFieldName.replace(/\//g, "::") + "-ok")
									.doPress()
						...
						assertions: {
							...
							iSeeVHDialog: function(sFieldName) {
								return OpaBuilder.create(this)
									.hasId(FilterBarVHDId + sFieldName.replace(/\//g, "::") + "-dialog")
									.description("Seeing value help dialog for field '" + sFieldName + "'")
									.execute();
							},
							iSeeVHDTable: function(sFieldName) {
								return OpaBuilder.create(this)
									.hasId(FilterBarVHDId + sFieldName.replace(/\//g, "::") + "::Table")
									.has(function(oTable) {
							...
							iCheckVHDTableRows: function(sFieldName, iExpectedNumberOfRows) {
								return OpaBuilder.create(this)
									.hasId(FilterBarVHDId + sFieldName.replace(/\//g, "::") + "::Table")
									.has(function(oTable) {
										return oTable.getItems().length === iExpectedNumberOfRows;
									})
									.description(
										"Seeing table of value help dialog for field '" +
											sFieldName +
											"' with " +
							...
							iSeeFieldInVHDFilterBar: function(sFieldName, sFilterBarField, oExpectedState) {
								return OpaBuilder.create(this)
									.hasId(
										FilterBarVHDId + sFieldName.replace(/\//g, "::") + "::FilterBar::FilterField::" + sFilterBarField
									)
							...
							_checkVHDSearchFieldIsVisible: function(sFieldName, bVisible) {
								var sVHFilterBarId = ViewId + "--" + FilterBarVHDId + sFieldName.replace(/\//g, "::") + "::FilterBar";
								return OpaBuilder.create(this)
									.hasId(sVHFilterBarId)
									.has(function(oVHFilterBar) {
										return bVisible
											? !!oVHFilterBar.getBasicSearchField()
											: !oVHFilterBar.getBasicSearchField() || !oVHFilterBar.getBasicSearchField().getVisible();
									})
									.mustBeVisible(false)
									.description(
							...
							iCheckVHDSearchFieldIsVisible: function(sFieldName, bVisible) {
								return this._checkVHDSearchFieldIsVisible(
									sFieldName.replace(/\//g, "::"),
									bVisible === undefined ? true : bVisible
								);
							},
							iSeeVHDFilterBar: function(sFieldName) {
								return OpaBuilder.create(this)
									.hasId(FilterBarVHDId + sFieldName.replace(/\//g, "::") + "::FilterBar")
