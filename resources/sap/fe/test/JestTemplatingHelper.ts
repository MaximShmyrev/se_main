...
export const compileCDS = function(sCDSUrl: string) {
	const cdsString = fs.readFileSync(sCDSUrl, "UTF8");
	...
	let edmxUrl = sCDSUrl.replace(".cds", ".xml");
	edmxUrl = edmxUrl.replace(/\/([^\/]*)$/, "/gen/" + "$1");
	// if the gen folder does not exist build it
	const dir = edmxUrl.slice(0, edmxUrl.lastIndexOf("/"));
...
export const getFakeDiagnostics = function(): IDiagnostics {
	...
	return {
		...
		checkIfIssueExists(issueCategory: IssueCategory, issueSeverity: IssueSeverity, details: string): any[] {
...
export const getConverterContext = function(
	...
	const dataModelPath = getDataModelObjectPathForProperty(entitySet, entitySet);
...
export const getDataModelObjectPathForProperty = function(entitySet: EntitySet, property?: Property): DataModelObjectPath {
