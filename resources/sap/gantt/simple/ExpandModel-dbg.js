...
sap.ui.define([
	...
	ExpandModel.prototype.isTableRowHeightNeedChange = function(bExpanded, oTable, aSelectedIndices, oPrimaryScheme, oExpandScheme) {
		...
		for (var iIndex = 0; iIndex < aSelectedIndices.length; iIndex++) {
			...
			aExpandableShapes.forEach(function(oMainshape){ // eslint-disable-line
				...
				var aChildWithLevels = GanttUtils.calculateLevelForShapes(aChild, "time", "endTime");
	...
	ExpandModel.prototype.getCalculatedRowHeight = function(oRowSettings, iBaseRowHeight, oTable) {
		...
		if (this.hasExpandedRows()) {
			...
			if (aItems) {
				...
				if (oGantt.getExpandedRowHeight() === undefined || isNaN(oGantt.getExpandedRowHeight())){
					iResult = iBaseRowHeight * (oMain.value.rowSpan + oMain.value.numberOfSubRows) ;
				} else {
					iResult = oGantt.getShowParentRowOnExpand() ? (iBaseRowHeight * oMain.value.rowSpan) + (oMain.value.numberOfSubRows * oGantt.getExpandedRowHeight()) : (oMain.value.numberOfSubRows * oGantt.getExpandedRowHeight());
				}
			}
		}
		return iResult;
	...
	ExpandModel.prototype.getRowYCenterByUid = function(sUid, iMainRowYCenter, sSchemeKey, iExpandIndex) {
		...
		for (var iIndex = 0; iIndex < aItems.length; iIndex++) {
			...
			if (oItem.scheme === sScheme) {
				...
				var iBaseRowHeight;
				if (aYAxis.length === 1  && oItem.metadata.main){
					iBaseRowHeight = this.getBaseRowHeight();
				} else if (aYAxis.length === 1  && !oItem.metadata.main){
					aYAxis[0] = aYAxis[0] - (this.getBaseRowHeight() * iIndex);
					iBaseRowHeight = aYAxis[0] - this.getBaseRowHeight();
				 } else {
					iBaseRowHeight = aYAxis[1] - aYAxis[0];
				}
				var fRowYCenter = 0, that = this;
				var iShapeHeight = iBaseRowHeight * oItem.metadata.rowSpan;
				var aRowYCenters = aYAxis.map(function(iValue){ //eslint-disable-line
					if (aYAxis.length === 1 && !oItem.metadata.main) {
						if (aYAxis[0] > 50){
							fRowYCenter = iValue + (iShapeHeight / 2) + that.getBaseRowHeight() - (aYAxis[0] - 50);
						} else {
							fRowYCenter = iValue + (iShapeHeight / 2) + that.getBaseRowHeight() + (50 - aYAxis[0]);
						}
						aYAxis[0] = aYAxis[0] + (that.getBaseRowHeight() * iIndex);
					} else {
						fRowYCenter = iValue + (iShapeHeight / 2);
					}
					return fRowYCenter;
				});
	...
	ExpandModel.prototype.collectExpandedBgData = function(aRowUid, iExpandedRowHeight) {
