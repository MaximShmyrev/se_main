...
sap.ui.define([
			...
			if (vType === RELATION_TYPE.FinishToFinish) {
				...
				if (x1 <= x2) {
					if (Core.getConfiguration().getRTL() ? this.getLShapeForTypeSF() : this.getLShapeForTypeFS()) {
						...
						fnCalculate = this.calcSRlsPathD; mArguments.push(nRowHeight, vType);
					}
				} else if (x1 > x2) {
					fnCalculate = this.calcSRlsPathD; mArguments.push(nRowHeight, vType);
				}
	 ...
	 */
	Relationship.prototype.calcSRlsPathD = function (x1, y1, x2, y2, nRowHeight,vType) {
		var x3 = vType == 1 ? x1 + 2 * ARROW_SIZE : x1 - 2 * ARROW_SIZE;
		var y3 = (y1 < y2) ? y1 + nRowHeight / 2 : y1 - nRowHeight / 2;
		var x4 = vType == 1 ? x2 - 2 * ARROW_SIZE : x2 + 2 * ARROW_SIZE;
	...
	Relationship.prototype.renderRelationship = function (oRm, mAnchors) {
		...
		oRm.attr("d", this.getArrowPathD(this.getD()));
		oRm.openEnd().close("path");
