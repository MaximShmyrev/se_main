...
sap.ui.define([
	...
	var RenderUtils = {
		...
		renderElementTitle : function(oRm, oElement) {
			...
			if (sTitle) {
				...
				var mTextSettings = {
					...
					textAnchor: oElement.getHorizontalTextAlignment(),
					verticalTextAlignment: oElement.getVerticalTextAlignment()
				};
					if (mTextSettings.textAnchor === "Start") {
						mTextSettings.x = Core.getConfiguration().getRTL() ? oElement.getX() + oElement.getWidth() : oElement.getX() + iCornerPaddingPixel;
						this.setVerticalAlignment(oElement, mTextSettings);
					} else if (mTextSettings.textAnchor === "End") {
						mTextSettings.x = Core.getConfiguration().getRTL() ? oElement.getX() + iCornerPaddingPixel : oElement.getX() + oElement.getWidth();
						this.setVerticalAlignment(oElement, mTextSettings);
					} else if (mTextSettings.textAnchor === "Middle") {
						mTextSettings.x = oElement.getX() + oElement.getWidth() / 2;
						this.setVerticalAlignment(oElement, mTextSettings);
					} else if (mTextSettings.textAnchor === "Ribbon") {
						Core.getConfiguration().getRTL() ? this.renderRepetitiveTextRTL(oElement, mTextSettings) : this.renderRepetitiveTextLTR(oElement, mTextSettings);
						this.setVerticalAlignment(oElement, mTextSettings);
					}
		...
		renderRepetitiveTextLTR: function(oElement, mTextSettings) {
			mTextSettings.textAnchor = "Start";
			mTextSettings.x = oElement.getX();
		...
		renderRepetitiveTextRTL: function(oElement, mTextSettings) {
			mTextSettings.textAnchor = "Start";
			mTextSettings.x = oElement.getX() + oElement.getWidth();
		...
		renderMainRowShape : function(oRm, oShape, mOption, bShowParentRow) {
			...
			if ((!Object.keys(mOption.expandModel.mExpanded).includes(mOption.rowUid) && !mOption.rowExpanded) || bShowParentRow){
				oShape.renderElement(oRm, oShape, null);
			}
		...
		renderExpandShapesIfNecessary : function(oRm, oMainShape, mOption, bShowParentRow) {
			...
			var fnRenderExpandShape = function(aShapes) {
				...
				aExpandedShapes.forEach(function(oShape, iIndex){
					...
					var iRowYCenter = bShowParentRow ?  mOption.expandModel.getRowYCenterByUid(mOption.rowUid, null, oShape.getScheme(), iIndex) : mOption.expandModel.getRowYCenterByUid(mOption.rowUid, null, oShape.getScheme(), iIndex) - oMainShape._iBaseRowHeight;
