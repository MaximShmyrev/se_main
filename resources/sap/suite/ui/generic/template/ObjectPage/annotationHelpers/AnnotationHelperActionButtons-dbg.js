...
sap.ui.define([
	...
	function isEditButtonRequired(oInterface, mRestrictions, oEntitySet, bParameterEdit, aAncestorTreeNodes){
		var oModel = oInterface.getInterface(0).getModel();
		var oParentEntitySet, sParentEntityName, mParentUpdateRestriction, bUpdateRestriction, bFlag = true;
		for (var i = 0; i < aAncestorTreeNodes.length; i++) {
			if (aAncestorTreeNodes[i].level > 0) {
				sParentEntityName = aAncestorTreeNodes[i].entitySet;
				oParentEntitySet = oModel.getODataEntitySet(sParentEntityName);
				mParentUpdateRestriction = oParentEntitySet["Org.OData.Capabilities.V1.UpdateRestrictions"];
				bUpdateRestriction = AnnotationHelper.areBooleanRestrictionsValidAndPossible(oInterface, mParentUpdateRestriction, oParentEntitySet, "Updatable", undefined, bParameterEdit);
				// if parent pages has update restriction do not show Edit button in child page
				if (mParentUpdateRestriction && !bUpdateRestriction) {
					bFlag = false;
					break;
				}
			}
		}
		return bFlag && AnnotationHelper.areBooleanRestrictionsValidAndPossible(oInterface, mRestrictions, oEntitySet, "Updatable", undefined, bParameterEdit);
	}
	...
	function getEditActionButtonVisibility(oInterface, mRestrictions, oEntitySet, oAppComponent, oTreeNode, aAncestortreeNodes) {
		...
		var oParentEntityset, sParentEntity, mParentUpdateRestriction;
		var aParentPath = [], sParentPath = "";
		var oModel = oInterface.getInterface(0).getModel();
		aAncestortreeNodes.forEach( function (oAncestorTree) {
			if (oAncestorTree.level > 0) {
				sParentEntity = oAncestorTree.entitySet;
				oParentEntityset = oModel.getODataEntitySet(sParentEntity);
				mParentUpdateRestriction = oParentEntityset["Org.OData.Capabilities.V1.UpdateRestrictions"];
				aParentPath.push(getParentPathExpression(oInterface, oParentEntityset, mParentUpdateRestriction, "Updatable", oAncestorTree));
			}
		});
		if (aParentPath.length > 1) {
			sParentPath = fnCombineByAnd.apply(null, aParentPath);
		} else if (aAncestortreeNodes.length && aParentPath.length === 1) {
			sParentPath = aParentPath[0];
		}
