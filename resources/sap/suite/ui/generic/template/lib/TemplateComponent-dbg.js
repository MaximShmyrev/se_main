sap.ui.define(["sap/ui/core/CustomizingConfiguration",
	...
	"sap/suite/ui/generic/template/js/placeholderHelper"
			...
			reuseComponentHelper, CustomData, StableIdDefinition, StableIdHelper, FeLogger, AnnotationHelper, extend, deepExtend, CacheHelper, FeError, placeholderHelper) {
		"use strict";
		...
		function createParameterModel(sEntityType, oComponentRegistryEntry, oMetaModel, fnTemplateSpecificParameters) {
			...
			var oModel = oComponentRegistryEntry.oAppComponent.getModel();
		...
		function fnCreateEmbeddedComponents(oComponentRegistryEntry){
			...
			oTreeNode.willBeDisplayed.then(oComponentRegistryEntry.oTemplateContract.oBusyHelper.setBusy.bind(null, oComponentRegistryEntry.reuseComponentsReady));
		}
				...
				if (oComponentRegistryEntry){
					...
					if (oModel) {
						...
						createXMLView(oComponentRegistryEntry, aCacheKeys).loaded().then(function(oView) {
							...
							var oTreeNode = oComponentRegistryEntry.oTemplateContract.mRoutingTree[oComponentRegistryEntry.route];
							var oTemplatePrivateGlobalModel = oComponentRegistryEntry.oTemplateContract.oTemplatePrivateGlobalModel;
							var aConditionToRemovePlaceholder = ['FE-VIEW-CREATED','FE-VIEW-CREATED-FIRSTTIMEONLY'];
							placeholderHelper.resetPlaceHoldersForRoute(oTemplatePrivateGlobalModel,oTreeNode.sRouteName,aConditionToRemovePlaceholder);
						}, function(oError){
