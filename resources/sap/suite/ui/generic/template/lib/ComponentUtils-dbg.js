sap.ui.define(["sap/ui/base/Object",
	...
	], function(BaseObject, ManagedObjectModel, controlHelper, FeLogger, AnnotationHelper, CRUDActionHandler, extend, isEmptyObject, FeError) {
	"use strict";
	...
	function getMethods(oComponent, oComponentRegistryEntry) {
		...
		function getRootAncestorDetails() {
		...
		function fnStateChanged(){
			oComponentRegistryEntry.oStatePreserverPromise.then(function(oStatePreserver){
		...
		function fnLayoutChanged(sLastLayout, sCurrentLayout){
			if (oComponentRegistryEntry.methods.getTableForChildContext){
				...
				if (oSmartTable){
					...
					if (controlHelper.isMTable(oTable)){ // currently only mTable is supported
		...
		return {
			...
			getRootAncestorDetails: getRootAncestorDetails,
