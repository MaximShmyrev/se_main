...
sap.ui.define(["sap/ui/base/Object",
	...
	"sap/suite/ui/generic/template/js/placeholderHelper"
			...
			TemplateComponent, ControlPersonalizationAPI, placeholderHelper) {
	"use strict";
	...
	function getMethods(oTemplateContract, oNavigationControllerProxy) {
		...
		// sPlaceholderValue holds the placeholder information from the URL
		...
		function fnInitSession(){
			...
			oTemplateContract.myIntentPromise = sap.ushell && sap.ushell.Container && new Promise(function(fnResolve){
		...
		// return a Promise that is eresolved after the following has happened:
		...
		function fnAdaptAppStates(oIdentity, oTargetIdentity){
			...
			return oTemplateContract.oFlexibleColumnLayoutHandler.getAppStatesPromiseForNavigation(oIdentity, oTargetIdentity);
		}
		...
		function fnNavigateToRoute(oOptions){
			...
			if (iMode >= 0){
				fnSetPlaceholderForIdentity(oRoutingOptions.identity); // set placeholder as early as possible
		...
		function getSwitchToSiblingPromise(oSiblingContext, iDisplayMode){
			...
			var oSiblingKeysPromise = oTemplateContract.oApplicationProxy.fillSiblingKeyPromise(oCurrentIdentity.treeNode, oCurrentIdentity.keys, oTargetIdentity.keys);
			return oSiblingKeysPromise.then(function(oTargetNode){
		...
		function fnSetPlaceholderForIdentity(oIdentity){
			if (!sPlaceholderValue){
				return;
			}
			var oFlexibleColumnLayoutHandler = oTemplateContract.oFlexibleColumnLayoutHandler;
			var oTemplatePrivateGlobalModel = oTemplateContract.oTemplatePrivateGlobalModel;
			var oRoutingTree = oTemplateContract.mRoutingTree;
			var aConditionToIgnorePlaceholder = ["FE-VIEW-CREATED","FE-VIEW-CREATED-FIRSTTIMEONLY","FE-HEADER-LOADED-FIRSTTIMEONLY","FE-DATA-LOADED-FIRSTTIMEONLY"];
			placeholderHelper.setPlaceholder(oIdentity, oFlexibleColumnLayoutHandler, oTemplatePrivateGlobalModel, oRoutingTree, aConditionToIgnorePlaceholder);
		}
		...
		function fnPrepareHostView(oTreeNode, isComponentPreloaded){
			...
			var oHostController = oHostView.getController();
			oHostController.setRouteName(oTreeNode.sRouteName);
			var oPlaceholderInfo = sPlaceholderValue && oTreeNode.behaviour.getPlaceholderInfo();
			var oPlaceholderPromise = oPlaceholderInfo && oPlaceholderInfo.getPlaceholderPage(oHostView.getId());
			if (oPlaceholderPromise){
				oPlaceholderPromise.then(function(oPlaceholderPage){
					var oNavContainer = oHostView.byId("placeholderHost");
					oNavContainer.addPage(oPlaceholderPage);
					oNavContainer.to(oPlaceholderPage);
				});
			}
		...
		function fnHandleBeforeRouteMatched(oEvent){
			...
			fnSetPlaceholderForIdentity(oCurrentIdentity);
		...
		function fnHandleRouteMatchedImpl() {
			...
			var fnAfterAlternateIdentityIsFound = function(oAlternativeIdentityInfo){
				if (oAlternativeIdentityInfo){ // then one of the cases described above occurs
					var oOptions = {
						identity: oAlternativeIdentityInfo.identity,
						mode: 1,
			...
			if (oActivationInfo.bIsBack) {
				oTemplateContract.oBusyHelper.setBusy(getAlternativeIdentityPromise(oCurrentIdentity).then(fnAfterAlternateIdentityIsFound));
			} else {
		...
		// Analyze routing structure and initialize the router
		oTemplateContract.oBusyHelper.setBusy((getParsedShellHashFromFLP() || Promise.resolve()).then(function(oResults){
			...
			if (oParsedUrl && oParsedUrl["sap-ui-xx-placeholder"]){
				sPlaceholderValue = oParsedUrl["sap-ui-xx-placeholder"][0];
				if (sPlaceholderValue === "FE-VIEW-CREATED" || sPlaceholderValue === "FE-VIEW-CREATED-FIRSTTIMEONLY" || sPlaceholderValue === "FE-DATA-LOADED" || sPlaceholderValue === "FE-DATA-LOADED-FIRSTTIMEONLY" || sPlaceholderValue === "FE-HEADER-LOADED" || sPlaceholderValue === "FE-HEADER-LOADED-FIRSTTIMEONLY"){ 
					oTemplateContract.oTemplatePrivateGlobalModel.setProperty("/generic/placeholderValue",sPlaceholderValue);
				} else {
					oTemplateContract.oTemplatePrivateGlobalModel.setProperty("/generic/placeholdersShown", Object.create(null)); 
				}
			} else {
				oTemplateContract.oTemplatePrivateGlobalModel.setProperty("/generic/placeholdersShown", Object.create(null)); 
			}
			return routingHelper.generateRoutingStructure(oTemplateContract).then(fnInitSession); // initialize the first session;
		}));
