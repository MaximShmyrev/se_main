sap.ui.define(["sap/ui/base/Object",
	...
	], function(BaseObject, MessageUtils, testableHelper, FeLogger, extend, isEmptyObject, FeError, placeholderHelper) {
		"use strict";
		...
		function getMethods(oTemplateContract) {
			...
			function fnApplyBusy(bImmediate) {
				...
				if (bIsBusyNew || bImmediate) {
					...
					var bPhysicalBusyState = bIsBusyNew && placeholderHelper.isPlaceHolderInactive(oTemplateContract.oTemplatePrivateGlobalModel);
			...
			// If place holder display is changed this might also affect physical dissplay of busy indicator
			oTemplateContract.oTemplatePrivateGlobalModel.bindProperty("/generic/placeholdersShown").attachChange(fnApplyBusyImmediately);			
			...
			function fnModifyETagMessagesOnly(aTransientMessages) {
				...
				aTransientMessages.forEach(function(oMessage) {
					...
					if (oTechnicalDetails && oTechnicalDetails.statusCode === "412" && !oTechnicalDetails.headers["preference-applied"]) {
