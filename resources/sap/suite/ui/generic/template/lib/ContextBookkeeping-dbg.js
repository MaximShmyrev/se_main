		...
		function getMethods(oTemplateContract) {
			...
			function fnSetRemovalPromiseForChild(oContextData, oRemovalPromise, bIsCancellation, bIsRoot){
				if (!oContextData.oContextInfo.bIsCreate || !bIsCancellation) {
					oContextData.oRemovalPromise = bIsRoot ? oRemovalPromise.then(function(oResponse){
						return oResponse.context;
					}) : oContextData.oSiblingPromise;
				}
			...
			function getSiblingPromise(sContextPath) {
				...
				return oContextData ? getDraftSiblingPromise(oContextData.oContext) : createDraftSiblingPromise(oTemplateContract.oAppComponent.getModel(), sContextPath);
			}
			...
			function getAlternativeIdentityPromise(oIdentity){
				...
				return getAlternativeContextPromise(sMainPath).then(function(oAlternativeContext){
					...
					var iDisplayMode = oAlternativeContext.iDisplayMode;
					var bDisplayChecked = false;
					var aTargetKeys = ["", oDataModelHelper.analyseContext(oAlternativeContext.context).key];
			...
			function areTwoKnownPathesIdentical(sPath1, sPath2, bIsRoot, oHistoricIdentity, oNewIdentity) {
				return new Promise(function (fnResolve, fnReject) {
					...
					//swap, if oContextData1 doesn't hold a value and oContextData2 is available and is an active object
					if ((!oContextData1 || !oContextData1.oContextInfo) && (oContextData2 && !oContextData2.oContextInfo.bIsDraft)) {
					...
					function fnTestEquivalence (oContextData1, oContextData2) {
						...
						if (bIsRoot) {
							getAlternativeContextPromise(sCanonicalPath1).then(function (oAlternativeContextInfo) {
								fnResolve(!!oAlternativeContextInfo && oAlternativeContextInfo.context.getPath() === sCanonicalPath2);
							}, fnReject);
							return;
						}
					...
					//if oContextData1 holds context data of an active object and if oContextData2 is not available, get draftInfo of oContextData2, and if 
					...
					if (!oContextData1.oContextInfo.bIsDraft && (!oContextData2 || !oContextData2.oContextInfo)) {
