			...
			* @return Bool
			...
			function expandRootIfAllowed() {
				if (isContentIdReferencingAllowed()) {
					return oComponentUtils.getRootExpand();
                }
				return null;
			}
			...
			function fnCallEdit(oDraftMainTreeDetails, bUnconditional, oPrereadData) {
				...
				return new Promise(
					function(resolve, reject) {
						oServices.oTransactionController.editEntity(oMainTreeBindingContext, !bUnconditional, expandRootIfAllowed())
							.then(
			...
			fnEditEntityImpl = function(bUnconditional) {
				...
				if (bIsDraftEnabled) {
					...
					if (iViewLevel > 0) {
						oMainTreeLevelOneDetails = oComponentUtils.getRootAncestorDetails();
			...
			function fnGetMessagesFromContextFilter(oContextFilter, oMessageManager) {
				...
				return oMessageBinding.getCurrentContexts().map(function(oContext) {
			...
			function saveEntityImpl(resolve, reject, oCreateWithDialogFilters) {
				...
				aMessagesBeforeSave = fnGetMessagesFromContextFilter(oContextFilter, oMessageManager);
				...
				var fnSaveInternal = function(bStrict){
					...
					var oSubmitChange = oServices.oTransactionController.triggerSubmitChanges()
						...
						if (oMessageManager) {
							var aNewMessages = [];
							var aMessagesAfterSave = fnGetMessagesFromContextFilter(oContextFilter, oMessageManager);
				...
				var fnInvokeAction = function (bIsStrict, bWithWarningDialog, aContexts){
					...
					var fnDone = function(aResponses){
						...
						if (bSuccess){
							...
							if (bNavigationAllowed && oResponseContext && oResponseContext !== oActionContext && oResponseContext.getPath() !==
								"/undefined"){
								if (oSourceControl){
