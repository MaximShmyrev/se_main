...
sap.ui.define([
    ...
    */
    ...
    function render (oRenderManager, oContainer, sApplicationType, sUrl, sAdditionalInformation) {
        ...
        var oFlpUriParams = new UriParameters(document.URL);
        ...
        if (sApplicationType === "TR" ||
            oContainer.getIsPostCapability() &&  new UriParameters(sUrl).get("sap-iframe-hint") === "GUI" ) {
            if (oFlpUriParams.get("sap-post-params") === "true") {
                oContainer.setIframePostAllParams(true);
            }
        }
        ...
        if (bPostMechanismEnabled === true &&
            ((sApplicationType === "NWBC" || sApplicationType === "TR") || oContainer.getIsPostCapability() === true)) {
            ...
            if (aKeysArray.length > 0) {
                ...
                oCAService.getAppStateData(aKeysArray).then(function (aDataArray) {
                    ...
                    if (oDomRef.length > 0) {
                        oNewRenderManager = sap.ui.getCore().createRenderManager();
                        ApplicationContainer.prototype._buildHTMLElements(oNewRenderManager, oContainer, aDataArray, aInfoArray, sUrl, true, oFlpUriParams);

                        oNewRenderManager.flush(oDomRef[0]);
                        oNewRenderManager.destroy();
                    } else {
                        ApplicationContainer.prototype._buildHTMLElements(oRenderManager, oContainer, aDataArray, aInfoArray, sUrl, true, oFlpUriParams);
                    }

                    setTimeout(oContainer.oDeferredRenderer.resolve, 0);
                }, function (sError) {
                    ApplicationContainer.prototype._buildHTMLElements(oRenderManager, oContainer, undefined, aInfoArray, sUrl, true, oFlpUriParams);
                    setTimeout(oContainer.oDeferredRenderer.resolve, 0);
                });
            } else {
                ApplicationContainer.prototype._buildHTMLElements(oRenderManager, oContainer, undefined, aInfoArray, sUrl, false, oFlpUriParams);
                setTimeout(oContainer.oDeferredRenderer.resolve, 0);
            }
    ...
    function buildHTMLElements (oRenderManager, oContainer, aAdditionalDataArray, aInfoArray, sUrl, bIsDelayedRendering, oFlpUriParams) {
        ...
        if (oContainer.getIframePostAllParams() === true) {
            oContainer.setIframePostAllParams(false);
    ...
    ApplicationContainer.prototype.getDeffedControlCreation = function (sUrl) {
        if (this.oDeferredControlCreation) {
            return this.oDeferredControlCreation.promise();
        } else {
