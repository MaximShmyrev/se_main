    ...
    return BaseDialogController.extend("sap.ushell.applications.PageComposer.controller.ConfirmChangesDialog.controller", {
        ...
        onOverwriteChanges: function (oEvent) {
            return new Promise(function (resolve, reject) {
                var oController = this._oView.getController(),
                    oRepo = oController.getPageRepository(),
                    oPage = this._oView.getModel().getProperty("/page"),
                    oSimpleError = this._oView.getModel().getProperty("/simpleError"),
                    sStatusCode = oSimpleError.statusCode,
                    that = this;

                oEvent.getSource().getParent().close();

                if (sStatusCode === "412") {
                    oRepo.getPage(oPage.id).then(function (oNewPage) {
                        oPage.modifiedOn = oNewPage.modifiedOn;
                        oController.savePageAndUpdateModel(oPage).then(function () {
