...
sap.ui.define([
    ...
    return BaseController.extend("sap.ushell.applications.PageComposer.controller.PageDetail", {
        ...
        onInit: function () {
            ...
            this._oInvisibleMessageInstance = this.getOwnerComponent().getInvisibleMessageInstance();
        },
        ...
        onBreakpointChanged: function (event) {
            ...
            if (typeof event !== "undefined") {
                ...
                oModel.setProperty("/currentBreakpointIsS", bCurrentBreakpointIsS);
            } else {
                bCurrentBreakpointIsS = oModel.getProperty("/currentBreakpointIsS");
            }
        ...
        onUpdateSideContentVisibility: function () {
            var bCatalogsExpanded = this.getModel().getProperty("/catalogsExpanded");
            var oToggleCatalogsButton = this.getView().byId("toggleCatalogsButton");
            oToggleCatalogsButton.setPressed(bCatalogsExpanded);
            this.getModel().setProperty("/catalogsExpanded", !bCatalogsExpanded);
            var sMessage = bCatalogsExpanded
                ? this.getResourceBundle().getText("Message.Catalogs.nowBeingHidden")
                : this.getResourceBundle().getText("Message.Catalogs.nowBeingShown");
            this._oInvisibleMessageInstance.announce(sMessage, InvisibleMessageMode.Polite);
        },
        ...
        switchDynamicSideContentView: function () {
            var oDynamicSideContent = this.getView().byId("pageContent");
            var oToggleCatalogsButton = this.getView().byId("toggleCatalogsButton");
            oDynamicSideContent.toggle();
            var bShowCatalogs = oDynamicSideContent.isSideContentVisible();
            oToggleCatalogsButton.setPressed(bShowCatalogs);
            var sMessage = bShowCatalogs
                ? this.getResourceBundle().getText("Message.Catalogs.nowBeingShown")
                : this.getResourceBundle().getText("Message.Catalogs.nowBeingHidden");
            this._oInvisibleMessageInstance.announce(sMessage, InvisibleMessageMode.Polite);
        },
         ...
         * @returns {Promise<object>} Resolves with the loaded Page data.
         * @private
         */
        _loadPage: function (onCancel) {
            ...
            return Promise.all([
                ...
                new Promise(function (resolve, reject) {
                    this.getModel("PageRepository").read("/tileTypeSet", {
                        urlParameters: { $expand: "vizOptions/displayFormats/supported" },
                        success: resolve,
                        error: reject
                    });
                }.bind(this))
                ...
                .then(function (aPromiseResults) {
                    var oTransportComponent = aPromiseResults[0];
                    var oPageData = aPromiseResults[1];
                    var aPageRoles = aPromiseResults[2];
                    ...
                    if (!this.getModel().getProperty("/editMode")) {
                        ...
                        this.Page.refreshRoleContext();
                        oTransportComponent.setShowAssignButton(true); // show the "Assign" transport button on the "Transports" tab
                ...
                .catch(function (oError) {
                    if (oError instanceof Error) { // prevent logging "[object Object]"
                        ...
                        Log.error(
                            ((oError && oError.message) ? oError.message : "Unexpected error"),
                            "PageComposer/controller/PageDetail.controller.js"
                        );
                    }
        ...
        _tryEnablingEditMode: function (oPageData, oTransportComponent, onCancel) {
            return this._pageEditAllowed(oPageData).then(function (bEditAllowed) {
                ...
                if (!bEditAllowed) {
                    ...
                    oTransportComponent.setShowAssignButton(true); // hide the "Assign" transport button on the "Transports" tab
                    return Promise.resolve(oPageData);
                }
                this.checkShowEditDialog(oPageData, this._updatePageWithMetadata.bind(this), onCancel);
                oTransportComponent.setShowAssignButton(false); // show the "Assign" transport button on the "Transports" tab
                sap.ui.getCore().getMessageManager().removeAllMessages(); // must be called before "_collectMessages"
                ...
                return this._loadTileSelector().then(function () {
                    return Promise.resolve(oPageData);
                });
            }.bind(this));
        },
         ...
         *
         * @returns {Promise<undefined>} Resolves when the TileSelector is loaded.
         * @private
         */
        ...
        _updatePageWithMetadata: function (event) {
            ...
            if (this.getRootView().byId("editDialog")) {
                this.getRootView().byId("editDialog").close();
            }
        },
        ...
        onSave: function () {
            ...
            var fnSave = function (sClickedAction) {
                if (sClickedAction === MessageBox.Action.OK) {
                    ...
                    this.savePageAndUpdateModel(oPageData)
                        .then(function () {
                            return this.getPageRepository().getRoles(this.sPageID);
                        }.bind(this))
                        .then(function (aPageRoles) {
                            this._resetRolesModel({ available: aPageRoles.map(function (oPageRole) { return oPageRole.id; }) });
                            MessageToast.show(oResourceBundle.getText("Message.SavedChanges"), { closeOnBrowserNavigation: false });
                            this._setDirtyFlag(false);
                        }.bind(this))
            ...
            if (!this.byId("confirmChangesDialog")) {
                Fragment.load({
                    ...
                    this.getModel().setProperty("/simpleError", {
                        ...
                        pageNotFound: oSimpleError.pageNotFound
                    });
        ...
        _collectMessages: function () {
            var oMessages = this.Page.collectMessages(),
                aErrors = oMessages.errors.concat(this._validatePageInfoAndGetErrors()),
         ...
         * Return value is used in the message popover.
         *
         * @returns {sap.ui.core.message.Message[]} An array of message objects.
         * @private
         */
        _getPageAssignmentMessages: function () {
            ...
            if (sMissingAssignment === "role") {
                aInfos.push(new Message({
                    ...
                    message: oResources.getText("Message.NotAssignedToRoleInformation"),
                    description: oResources.getText("Message.NotAssignedToRoleInformationDetails")
                }));
            } else if (sMissingAssignment === "space") {
                aInfos.push(new Message({
                    type: MessageType.Information,
                    message: oResources.getText("Message.NotAssignedToSpaceInformation"),
        ...
        onOpenContextSelector: function () {
            this.showContextSelector(function (aSelectedRoleIDs) {
                this._resetRolesModel({ selected: aSelectedRoleIDs });
        ...
        removeVisualizationInSection: function (visualizationIndex, sectionIndex) {
            var sPath = "/page/sections/" + sectionIndex + "/viz",
                ...
                oSection = this.getView().byId("page").getSections()[sectionIndex],
                oPosition = oSection.getItemPosition(visualizationIndex);

            var oVizContext = aVisualizationsData.splice(visualizationIndex, 1);
            this.getModel().setProperty(sPath, aVisualizationsData);

            if (oVizContext[0].displayFormatHint === DisplayFormat.Compact) {
                MessageToast.show(this.getResourceBundle().getText("Message.LinkRemoved"));
            } else {
                MessageToast.show(this.getResourceBundle().getText("Message.TileRemoved"));
            }
         ...
         * @param {string} displayFormatHint The displayFormatHint the tile should be converted to.
         * @returns {boolean} true if the visualization was moved
         * @protected
         */
        moveVisualizationInSection: function (originalVisualizationIndex, newVisualizationIndex, originalSectionIndex, newSectionIndex, displayFormatHint) {
            ...
            // If necessary convert the displayFormatHint
            var sOriginalDisplayFormatHint = oContent[0].displayFormatHint;
            // Do not convert flatWide to flat
            var bConversionNecessary = displayFormatHint && (sOriginalDisplayFormatHint || "").indexOf(displayFormatHint) !== 0;
            if (bConversionNecessary) {
                oContent[0].displayFormatHint = displayFormatHint;
            }
            ...
            var sInvisibleMessage;
            if (bConversionNecessary) {
                if (sOriginalDisplayFormatHint === DisplayFormat.Compact) {
                    sInvisibleMessage = ushellResources.i18n.getText("PageRuntime.Message.LinkConverted");
                } else {
                    sInvisibleMessage = ushellResources.i18n.getText("PageRuntime.Message.TileConverted");
                }
            } else if (displayFormatHint === DisplayFormat.Compact) {
                sInvisibleMessage = ushellResources.i18n.getText("PageRuntime.Message.LinkMoved");
            } else {
                sInvisibleMessage = ushellResources.i18n.getText("PageRuntime.Message.TileMoved");
            }
         ...
         *
         * @return {sap.ui.core.message.Message[]} An array of message objects.
         * @private
         */
        ...
        /**
         * Validates the page title input field and returns an error message if it is not valid.
         * Return value is used in the message popover.
         *
         * @returns {sap.ui.core.message.Message|undefined} Error message or "undefined" if title is valid.
         * @private
         */
        _getPageTitleError: function () {
            ...
            if (!bIsValid) {
                oError = new Message({
                    ...
                    message: this.getResourceBundle().getText("Message.InvalidPageTitle"),
        ...
        /**
         * Validates the page description input field and returns an error message if it is not valid.
         * Return value is used in the message popover.
         *
         * @returns {sap.ui.core.message.Message|undefined} Error message or "undefined" if description is valid.
         * @private
         */
        _getPageDescriptionError: function () {
            ...
            if (!bIsValid) {
                oError = new Message({
                    type: MessageType.Error,
                    message: this.getResourceBundle().getText("Message.InvalidPageDescription"),
