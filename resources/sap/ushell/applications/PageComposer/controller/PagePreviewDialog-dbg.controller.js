...
sap.ui.define([
    ...
    function _filterRoles () {
        ...
        if (oPage) {
            ...
            oPage.getSections().forEach(function (oSection) {
                ...
                oSection.filterVisualizations(oFilter);
            });
        }
    }
     ...
     * @param {string[]} aSelectedRoleIDs Array of role IDs that are currently selected for display.
     * @private
     */
    function _setRoleContext (aRoles, aSelectedRoleIDs) {
        if (_oDialog) {
            var oModel = _oDialog.getModel("roleContext");
            aSelectedRoleIDs = aSelectedRoleIDs || oModel.getProperty("/selectedRoles") || [];
            aRoles = aRoles || oModel.getProperty("/allRoles") || [];

            var sCount = aSelectedRoleIDs.length + "";
            if (!aSelectedRoleIDs.length) {
                sCount = _sNoText;
            } else if (aSelectedRoleIDs.length === aRoles.length) {
                sCount = _sAllText;
            }

            oModel.setData({
                allRoles: aRoles,
                selectedRoles: aSelectedRoleIDs,
    ...
    var PagePreviewDialogController = Controller.extend("sap.ushell.applications.PageComposer.controller.PagePreviewDialog.controller", {
         ...
         *
         * @param {string[]} aSelectedRoleIDs selected role context.
         * @private
         */
        onRolesSelected: function (aSelectedRoleIDs) {
            _setRoleContext(null, aSelectedRoleIDs);
        },
