            ...
            Object.keys(oSite.vizTypes).forEach(function (sKey) {
                if (oSite.vizTypes[sKey]["sap.flp"] && oSite.vizTypes[sKey]["sap.flp"].vizOptions) {
                    ...
                    if (aSupportedDisplayFormats) {
                        var aMappedFormats = this._mapDisplayFormats(aSupportedDisplayFormats);
                        oSite.vizTypes[sKey]["sap.flp"].vizOptions.displayFormats.supported = aMappedFormats;
                    }
                    if (sDefaultDisplayFormat) {
                        var aMappedFormat = this._mapDisplayFormats([sDefaultDisplayFormat])[0];
                        oSite.vizTypes[sKey]["sap.flp"].vizOptions.displayFormats.default = aMappedFormat;
                    }
                }
            }.bind(this));
        }
