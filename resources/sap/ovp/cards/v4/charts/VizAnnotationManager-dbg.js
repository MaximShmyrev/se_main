...
sap.ui.define(["sap/ui/thirdparty/jquery", "sap/ovp/cards/v4/charts/Utils", "sap/ui/model/odata/CountMode",
    ...
    function (jQuery, Utils, CountMode, CardAnnotationHelper, NumberFormat, DateFormat,
        ...
        /*
         *  @param {boolean} reset - if true, reset the global formatter of the vizFrame charts when navigating out of the OVP application
         */
...
//										showScale: false,
        ...
        function buildVizAttributes(vizFrame, handler, chartTitle, oController) {
            ...
            entityTypeObject = oController.getMetaModel().getData()["$Annotations"];
            if (!entityTypeObject) {
            ...
            var oMetadata = getMetadata(dataModel, entitySet);
            chartAnno = oCardsModel.getProperty("/chartAnnotationPath");
            if (!chartAnno || !(chartContext = entityTypeObject[oCardsModel.getProperty("/entityType").$Type]["@" + chartAnno])) {
                Log.error(errorMessages.CARD_ANNO_ERROR + "in " + errorMessages.CARD_ANNO);
                return;
            }
            ...
            each(config.feeds, function (i, feed) {
                ...
                if (feed.type) {
                    ...
                    if (aFeedProperties.length) {
                        ...
                        var sEntitySetName = oCardsModel.getProperty("/entityType").$Type;
                        each(aFeedProperties, function (i, val) {
                            ...
                            var edmType = null;
                            if (entityTypeObject[sEntitySetName + "/" + property]) {
                                if (entityTypeObject[sEntitySetName + "/" + property]["@" + constants.LABEL_KEY_V4]) { //as part of supporting V4 annotation
                                    if (entityTypeObject[sEntitySetName + "/" + property]["@" + constants.LABEL_KEY_V4].String) {
                                        feedName = getLabelFromAnnotationPath(entityTypeObject[sEntitySetName + "/" + property]["@" + constants.LABEL_KEY_V4].String, vizFrame, oMetadata);
                                    } else if (entityTypeObject[sEntitySetName + "/" + property]["@" + constants.LABEL_KEY_V4].$Path) {
                                        feedName = entityTypeObject[sEntitySetName + "/" + property]["@" + constants.LABEL_KEY_V4].$Path;
                                    } else {
                                        feedName = entityTypeObject[sEntitySetName + "/" + property]["@" + constants.LABEL_KEY_V4]; //Fallback on the technical name in case sap:label is not maintained in the desired language
                                    }
                                } else if (entityTypeObject[sEntitySetName + "/" + property][constants.LABEL_KEY]) {
                                    feedName = entityTypeObject[sEntitySetName + "/" + property][constants.LABEL_KEY];
                                } else if (property) {
                                    feedName = property;
                                }
                                if (entityTypeObject[sEntitySetName + "/" + property][constants.TEXT_KEY_V4]) { //as part of supporting V4 annotation
                                    textColumn = entityTypeObject[sEntitySetName + "/" + property][constants.TEXT_KEY_V4].String ? entityTypeObject[sEntitySetName + "/" + property][constants.TEXT_KEY_V4].String : entityTypeObject[sEntitySetName + "/" + property][constants.TEXT_KEY_V4].$Path;
                                } else if (entityTypeObject[sEntitySetName + "/" + property][constants.TEXT_KEY]) {
                                    textColumn = entityTypeObject[sEntitySetName + "/" + property][constants.TEXT_KEY];
                                } else if (property) {
                                    textColumn = property;
                                }
                            ...
                            if ((edmType === "Edm.DateTime" || edmType === "Edm.DateTimeOffset") && textColumn === property) {
                                ...
                                if (entityTypeObject[sEntitySetName + "/" + property] && entityTypeObject[sEntitySetName + "/" + property][constants.TEXT_KEY_V4] && entityTypeObject[sEntitySetName + "/" + property][constants.TEXT_KEY_V4][constants.TEXT_ARRANGEMENT_ANNO]) {
                                    ...
                                    oTextArrangement = entityTypeObject[sEntitySetName + "/" + property][constants.TEXT_ARRANGEMENT_ANNO];
                                    sTextArrangementType = oTextArrangement && oTextArrangement.EnumMember;

                                    //Priority 2 - Text Level
                                    if (!sTextArrangementType) {
                                        oTextArrangement = entityTypeObject[sEntitySetName + "/" + property][constants.TEXT_KEY_V4][constants.TEXT_ARRANGEMENT_ANNO];
                                        sTextArrangementType = oTextArrangement && oTextArrangement.EnumMember;
                                    }
                                ...
                                if (dimensionArrayNames[j] === dimensions[k]) {
                                    for (var key in oMetadata) {
                                        if (annotations[oEntityType.$Type + "/" + key]) {
                                            var propertyName = annotations[oEntityType.$Type + "/" + key]["@" + constants.LABEL_KEY_V4] || annotations[oEntityType.$Type + "/" + key][constants.LABEL_KEY] || annotations[oEntityType.$Type + "/" + key][constants.NAME_KEY] || annotations[oEntityType.$Type + "/" + key][constants.NAME_CAP_KEY] || key;
